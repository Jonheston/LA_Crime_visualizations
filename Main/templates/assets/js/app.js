...
xText
  .append("text")
  .attr("y", -26)
  .attr("data-name", "Percent NonHisp Black")
  .attr("data-axis", "x")
...
xText
  .append("text")
  .attr("y", 0)
  .attr("data-name", "Percent NonHisp White")
  .attr("data-axis", "x")
...
.append("text")
.attr("y", 52)
.attr("data-name", "Total Population")
.attr("data-axis", "x")
.attr("class", "aText inactive x")
...
yText
  .append("text")
  .attr("y", -26)
  .attr("data-name", "Violent Crime Rate")
  .attr("data-axis", "y")
...
// ========================
d3.csv("assets/data/crime_pov_census_2017.csv", function(data) {
  visualize(data);
});
...
select.selectAll("option")
  ...
  .enter()
    .append("option")
    .attr("value", function (d) { return d.zip; })
...
function visualize(theData) {
  var curX = "Percent NonHisp Black";
  var curY = "Violent Crime Rate";
  ...
  // b. Similarly change the min and max for y
  function yMinMax() {
    // min will grab the smallest datum from the selected column.
  ...
  // We append the circles for each row of data (or each zip, in this case).
  theCircles
    .append("circle")
    ...
    .attr("class", function(d) {
      return "zipCircle " + d.abbr;
    })
    // Hover rules
    .on("mouseover", function(d) {
      // Show the tooltip
      toolTip.show(d);
      // Highlight the zip circle's border
  ...
  d3.selectAll(".aText").on("click", function() {
    ...
    if (self.classed("inactive")) {
      ...
      if (axis === "x") {
        ...
        // We need change the location of the zip texts, too.
        d3.selectAll(".zipText").each(function() {
          // We give each zip text the same motion tween as the matching circle.
          d3
            .select(this)
            .transition()
            .attr("dx", function(d) {
